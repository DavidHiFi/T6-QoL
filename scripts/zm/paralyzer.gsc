// ============================================================================
//  paralyzer.gsc  -  BURIED'S PARALYZER, OFF BURIED                 (v2.17.35)
//
//  maps\mp\zombies\_zm_weap_slowgun.gsc, verbatim, with its seven loadfx()
//  calls repointed and a two-map gate on init(). Everything else - the damage
//  response, the anim-rate slow, the spawn and death callbacks, the sound
//  clientfield - is Treyarch's own code unchanged.
//
//  🛑 WHY A COPY AT ALL. The core script ships only in zm_buried.ff. A
//  qualified reference to it from a root script resolves at SCRIPT LOAD time,
//  so it would be an Unresolved external on every other map - the same class
//  as ERROR_CATALOGUE item 3.
//
//  🛑 WHY THE loadfx CALLS HAD TO MOVE. Stock loads seven weapon/paralyzer/*
//  effects that live in zm_buried.ff alone, and OpenAssetTools cannot read or
//  write FxEffectDef at all, so they cannot be carried into mod.ff. A weapon
//  def's fx STRING resolves leniently - this mod's Olympia has shipped on six
//  maps for weeks naming a flash in none of them - but a GSC loadfx does not.
//  The replacements are the Winter's Howl freeze effects this mod already
//  ships as raw .efx inside mod.iwd, which is how the T5 wonder weapons load
//  their 27 effects on every map:
//
//      zombie_slowgun_explosion     weapon/freeze_gun/fx_freezegun_shatter
//      zombie_slowgun_explosion_ug  weapon/freeze_gun/fx_exp_freezegun_impact
//      zombie_slowgun_sizzle        maps/zombie/fx_zombie_freeze_md
//      zombie_slowgun_sizzle_ug     maps/zombie/fx_zombie_freeze_torso
//      player_slowgun_sizzle*       weapon/freeze_gun/fx_freezegun_smoke_cloud
//
//  🛑 THE TWO MAPS IT REFUSES, AND THE NUMBERS.
//    zm_buried  ships the real gun and calls the core init itself. Running
//               both registers the same five clientfields twice - fatal.
//    zm_tomb    the ACTOR clientfield set is 31 of 32 stock on every Origins
//               dump (awk over T6-Data-Archive\ZM\Clientfields), and this gun
//               wants 8 actor bits: slowgun_fx 3 + anim_rate 5. TranZit and
//               Die Rise are 4-5, Nuketown 4, Mob 11-13, so those clear it.
//
//  📝 THE CLIENT HALF IS STOCK'S. clientscripts\mp\zombies\_zm_weap_slowgun
//  .csc registers the client side of these fields from its own
//  is_weapon_included check, so including the weapon is what switches it on -
//  there is no client twin of this file to keep in step.
// ============================================================================
...
init()
{
    //  See the banner: Buried runs the core script itself, and Origins has one
    //  spare actor clientfield bit against this gun's eight.
    ...
    level._effect["zombie_slowgun_explosion"] = loadfx( "weapon/freeze_gun/fx_freezegun_shatter" );
    level._effect["zombie_slowgun_explosion_ug"] = loadfx( "weapon/freeze_gun/fx_exp_freezegun_impact" );
    level._effect["zombie_slowgun_sizzle"] = loadfx( "maps/zombie/fx_zombie_freeze_md" );
    level._effect["zombie_slowgun_sizzle_ug"] = loadfx( "maps/zombie/fx_zombie_freeze_torso" );
    level._effect["player_slowgun_sizzle"] = loadfx( "weapon/freeze_gun/fx_freezegun_smoke_cloud" );
    level._effect["player_slowgun_sizzle_ug"] = loadfx( "weapon/freeze_gun/fx_freezegun_smoke_cloud" );
    level._effect["player_slowgun_sizzle_1st"] = loadfx( "weapon/freeze_gun/fx_freezegun_smoke_cloud" );
...
reset_anim()
{
    ...
    if ( is_true( self.is_traversing ) )
    {
        ...
        {
            self.no_restart = 1;
            animstate = animstate + "_no_restart";
        }
...
zombie_paralyzed( player, upgraded )
{
    ...
    if ( self.slowgun_anim_rate <= 0.1 || insta && self.slowgun_anim_rate <= 0.5 )
    {
        ...
        damage = damage * randomfloatrange( 0.667, 1.5 );
        damage = damage * self.paralyzer_damaged_multiplier;
        ...
        self.paralyzer_damaged_multiplier = self.paralyzer_damaged_multiplier * 1.15;
    ...
    if ( isdefined( ground ) && isdefined( ground.team ) && ground.team == level.zombie_team )
    {
        ...
        while ( !isdefined( ground ) || isdefined( ground.team ) && ground.team == level.zombie_team )
        {
            ...
            if ( isdefined( ground ) )
            {
                ...
                {
                    level.ghost_head_damage = level.ghost_head_damage * 1.5;
