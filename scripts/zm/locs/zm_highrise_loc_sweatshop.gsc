...
precache()
{
	...
	precachemodel( "p6_zm_hr_bldg2_rest_pillar01" );
...
swap_buildable_tables()
{
	...
	// zm_qol: repointed from scripts\zm\replaced\_zm_buildables_pooled (not ported -
	// it #includes a Buried-only stock module). Same code, carried in loc_common.
	scripts\zm\locs\loc_common::swap_buildable_fields(level.buildable_stubs[0], level.buildable_stubs[1]);
}