...
// ============================================================================
//  zmqol_mgun_swell_init / zmqol_mgun_swell_swap  -  MOON'S BLOAT, THE WAY
//  THE WORKING DER RIESE PORT DOES IT                          (v2.17.46)
// ----------------------------------------------------------------------------
//  User, 2026-09-26: a friend's Wave Gun build for Zombies Declassified's Der
//  Riese (build 11, modding-jobs\wavegun-build11-001) swells zombies properly;
//  make ours do the same, as close to BO1 Moon as possible.
//
//  WHAT THAT BUILD DOES, MEASURED (decompiled and disassembled, not guessed):
//    * Its vertex shader is Moon's, one line: worldPos += normal * swell.
//      Byte-for-byte the same instruction stream as our own v2.15.49 shaders.
//    * At the kill it swaps the body and every attached head to a COPY of the
//      model whose vertex normals are WELDED - every vertex sharing a position
//      gets the average normal - and whose materials run that shader.
//
//  The weld is the whole trick. A stock mesh splits the normal at every hard
//  edge, so pushing each vertex along its own normal tears the surface open
//  there (c_zom_zombie1_body01: 115 seams, most over 60 degrees) - which is
//  why v2.15.51 retreated to a stomach-only ball. Welded, the entire body and
//  head inflate as one skin, which is what Moon looks like.
//
//  The copies (zqwg_<stock name>) are generated by
//  modding-jobs\wavegun-build11-001\gen_swell_models.py and declared in
//  mod_wavegun_swell.zone. Only the current map's set is precached: the model
//  index is a configstring pool and 70 extra models is the measured headroom
//  on Town (wavegun-swell-003). The biggest set here is 16.
//
//  A body with no copy (a gib form, zombie9 on grief) keeps its model and the
//  gun still kills it; only the swell is missing. No gib happens on a Wave
//  Gun kill anyway - no_gib/head_gibbed are set before the damage.
// ============================================================================
zmqol_mgun_swell_init()
{
    level.zmqol_mgun_swell = [];
    str_map = getdvar( "mapname" );
    a = [];

    if ( str_map == "zm_transit" )
    {
        a = strtok( "c_zom_zombie1_body01 c_zom_zombie1_body02 c_zom_zombie_head_a c_zom_zombie_head_k c_zom_zombie_head_l c_zom_zombie_head_n", " " );

        //  The two TranZit zones carry different families (so_zclassic:
        //  zombie2/3; so_zsurvival: zombie5/8 and the female heads d/f).
        if ( is_classic() )
            b = strtok( "c_zom_zombie2_body01 c_zom_zombie2_body02 c_zom_zombie2_body03 c_zom_zombie3_body01 c_zom_zombie3_body02 c_zom_zombie3_body03 c_zom_zombie3_body04 c_zom_zombie3_body05", " " );
        else
            b = strtok( "c_zom_zombie5_body01 c_zom_zombie5_body02 c_zom_zombie5_body03 c_zom_zombie8_body01 c_zom_zombie8_body02 c_zom_zombie8_body03 c_zom_zombie_head_d c_zom_zombie_head_d2 c_zom_zombie_head_f c_zom_zombie_head_f2", " " );

        for ( i = 0; i < b.size; i++ )
            a[ a.size ] = b[i];
    }
    else if ( str_map == "zm_nuked" )
        a = strtok( "c_zom_dlc0_zom_haz_body1 c_zom_dlc0_zom_haz_body2 c_zom_dlc0_zom_sol_body1 c_zom_dlc0_zom_solciv_body1 c_zom_dlc0_zom_haz_head_mask c_zom_dlc0_zom_haz_head_mask_blueeyes c_zom_dlc0_zom_head1 c_zom_dlc0_zom_head1_blueeyes c_zom_dlc0_zom_head2 c_zom_dlc0_zom_head2_blueeyes c_zom_dlc0_zom_head3 c_zom_dlc0_zom_head3_blueeyes c_zom_dlc0_zom_head4 c_zom_dlc0_zom_head4_blueeyes", " " );
    else if ( str_map == "zm_highrise" )
        a = strtok( "c_zom_zombie_civ_shorts_body c_zom_zombie_civ_shorts_body2 c_zom_zombie_civ_shorts_body3 c_zom_zombie_civ_shorts_body4 c_zom_zombie_civ_shorts_body5 c_zom_zombie_civ_shorts_body6 c_zom_zombie_scientist_body c_zom_zombie_soldier_body c_zom_leaper_body c_zom_zombie_chinese_head1 c_zom_zombie_chinese_head2 c_zom_zombie_chinese_head3 c_zom_zombie_chinese_head3_helmet c_zom_zombie_chinese_head4 c_zom_leaper_head", " " );
    else if ( str_map == "zm_prison" )
        a = strtok( "c_zom_guard_body c_zom_inmate_body1 c_zom_inmate_body2 c_viet_zombie_nohat c_zom_guard_hat c_zom_zombie_barbwire_head c_zom_zombie_hellcatraz_head c_zom_zombie_mask_head c_zom_zombie_mask_head_device c_zom_zombie_slackjaw_head", " " );

    for ( i = 0; i < a.size; i++ )
    {
        precachemodel( "zqwg_" + a[i] );
        level.zmqol_mgun_swell[ a[i] ] = "zqwg_" + a[i];
    }

    println( "[zm_qol] zapgun: " + a.size + " swell model(s) precached for " + str_map );
}

//  The friend's halo_wavegun_use_swell_models(), ours by name. Called on the
//  corpse right after the kill, before the expand clientfield goes up, so the
//  client's ramp drives the welded copy from its first frame.
zmqol_mgun_swell_swap()
{
    if ( !isdefined( level.zmqol_mgun_swell ) || isdefined( self.zmqol_mgun_swollen ) )
        return;

    self.zmqol_mgun_swollen = 1;
    a_model = [];
    a_tag = [];
    ...
    for ( i = 0; i < n; i++ )
    {
        a_model[i] = self getattachmodelname( i );
        a_tag[i] = self getattachtagname( i );
    }

    for ( i = 0; i < a_model.size; i++ )
    {
        str_swell = level.zmqol_mgun_swell[ a_model[i] ];

        if ( !isdefined( str_swell ) )
            continue;

        self detach( a_model[i], a_tag[i] );
        self attach( str_swell, a_tag[i], 1 );

        if ( isdefined( self.headmodel ) && self.headmodel == a_model[i] )
            self.headmodel = str_swell;

        if ( isdefined( self.hatmodel ) && self.hatmodel == a_model[i] )
            self.hatmodel = str_swell;
    }

    str_body = level.zmqol_mgun_swell[ self.model ];

    if ( isdefined( str_body ) )
...
microwavegun_fired( upgraded )
{
    ...
    // ------------------------------------------------------------------------
    //  🛑 v2.17.46 - NOT ON BRUTUS. He survives the first shot (it only takes
    //  the helmet, zmqol_brutus_ww_hit), and these three flags on a LIVE boss
    //  outlast it: nodeathragdoll would freeze whatever finally kills him in
    //  the last frame of his death anim. The boss hook handles him entirely.
    for ( i = 0; i < shot.enemies.size; i++ )
    {
        if ( isdefined( shot.enemies[i] ) && !is_true( shot.enemies[i].is_brutus ) )
        {
...
// ============================================================================
// ============================================================================
//  🌟 v2.17.46 - AND BRUTUS, THE SAME WAY THE OTHER THREE GUNS GOT HIM.
//
//  User, 2026-09-26, Docks survival: the Wave Gun "doesn't affect brutus".
//  Same flag, one AI further. Brutus sets ignore_enemy_count in brutus_spawn()
//  (_zm_ai_brutus.gsc:283), so get_round_enemy_array() never offered him to
//  the sizzle cone and the beam went straight through him - exactly the
//  denizen bug above. The Thundergun, Wunderwaffe and Winter's Howl hit this
//  in v1.93.0 and fixed it with srs_ww_target_array(), the full species array.
//
//  He is added by his OWN marker (is_brutus), not by dropping the filter:
//  the ghosts, the sloth and Mechz carry the same flag, and the gun is gated
//  off on Buried and Origins anyway. Once he is a candidate he reaches
//  microwavegun_sizzle_zombie(), whose FIRST check is level.zmqol_ww_boss_hit
//  - zm_prison.gsc's zmqol_brutus_ww_hit(), the two-shot helmet-then-kill the
//  other three guns already use. Nothing about Brutus is re-implemented here.
// ============================================================================
zmqol_ww_sizzle_target_list()
{
    ...
    for ( i = 0; i < a_ai.size; i++ )
    {
        ...
        if ( !( isdefined( a_ai[i].isscreecher ) && a_ai[i].isscreecher ) && !( isdefined( a_ai[i].is_brutus ) && a_ai[i].is_brutus ) )
            continue;
...
microwavegun_sizzle_zombie( player, sizzle_vec, index )
{
    ...
    if ( self.health <= 0 )
    {
        ...
        self zmqol_mgun_swell_swap();   // v2.17.46 - welded copies, the Der Riese build's route
        instant_explode = 0;
