// ============================================================================
//  _zm_weap_slowgun.csc  -  zm_qol's copy, for the Paralyzer off Buried
//                                                                   (v2.17.36)
//  CLIENT TWIN of scripts\zm\paralyzer.gsc. Stock's copy of this file ships
//  only in zm_buried.ff, so without this one the server would register five
//  clientfields off Buried that the client never did - slowgun_fx (actor 3 and
//  toplayer 1), anim_rate (actor 5 and allplayers 5) and sndParalyzerLoop -
//  and every player is dropped with EXE_CLIENT_FIELD_MISMATCH before the map
//  starts.
//
//  🌟 THE GATE IS STOCK'S OWN AND IT IS WHY THE TWO HALVES CANNOT DISAGREE.
//  is_weapon_included( "slowgun_zm" ) is true exactly where
//  boxfix.gsc::zmqol_paralyzer_register() included the gun, and false on
//  Origins where it refused - so the registration follows the weapon list on
//  both sides with no second map test to keep in step.
//
//  📝 THE _qol SUFFIX IS DELIBERATE. At the stock path this file would
//  REPLACE Buried's own copy, and Buried's Paralyzer would then run with this
//  mod's substituted freeze effects instead of its own. Under its own name
//  Buried is untouched. Nothing calls this automatically either - it is
//  invoked by zm_expanded.csc under the same map gate as the server half.
//
//  🛑 THE FIVE loadfx CALLS ARE REPOINTED, exactly as in the server copy.
//  weapon/paralyzer/* lives in zm_buried.ff alone and OpenAssetTools cannot
//  carry FxEffectDef, so they are the Winter's Howl freeze effects this mod
//  already ships as raw .efx in mod.iwd. Everything else is Treyarch's.
// ============================================================================
