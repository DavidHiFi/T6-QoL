-- ============================================================================
--  zm_qol - ui_mp\t6\hud\scoreboard.lua        v2.2.6
--
--  WHY THIS FILE EXISTS: the scoreboard title said "Survival - Green Run" while
--  the player was in DINER survival. User, 2026-08-23: *"the scoreboard for maps
--  still don't say the specific map name like how reimagined does ... for that
--  map i was in for that screenshot it would've been diner survival"*.
--
--  🛑 THE TITLE IS BUILT BY A **local** FUNCTION IN THIS FILE
--  (CreateScoreboardHeaderTitle), so there is no way to change it from a file we
--  already ship - the whole file has to be owned. Two sources were measured
--  before picking one:
--    * the stock patch_zm decompile - 🛑 UNSHIPPABLE. It does not even parse:
--      luaparse stops at "'=' expected near 'basicblock_79'" (line 687), and its
--      title function has the lossy line `f4_local5 = f4_local6` where a table
--      constructor should be. See [[t6-decompiles-are-lossy]].
--    * BO2-Reimagined's copy - parses clean, ships in a working mod, and already
--      implements exactly the behaviour the user asked for. That is the base.
--
--  🌟 THE MECHANISM, VERIFIED, NOT ASSUMED. Stock builds the title as
--  { GetZombieGameTypeName(gametype,map), " - ", mapsTable[map].col3 }:
--    * gametypestable.csv section 0 -> zstandard col 7 = ZMUI_STANDARD
--    * en_patch_zm.ff  -> ZMUI_STANDARD  = "Survival"
--    * there is NO ZMUI_STANDARD_zm_transit string, so element 1 stays "Survival"
--    * mapstable.csv   -> zm_transit col 3 = ZMUI_TRANSIT = "Green Run"
--  which is "Survival - Green Run", byte for byte what the screenshot shows.
--  The fix replaces element 3 with the START LOCATION instead of the map.
--
--  🌟 AND IT NEEDS NO NEW ASSET. The real stock zm/gametypestable.csv, dumped
--  out of patch_zm.ff with OpenAssetTools, ALREADY carries the row
--      5,10,zm_transit,diner,...,ZMUI_DINER,...
--  and en_patch_zm.ff already carries ZMUI_DINER = "Diner". Treyarch left both
--  in. (An older copy in t6-map-templates has no diner row - that copy is stale,
--  which is why the shipped fastfile was dumped rather than trusted.) The live
--  dvar was confirmed from the user's own console_zm.log: ui_zm_mapstartlocation
--  "diner" on the Diner run, "prison" on the Mob run.
--
--  📝 WHAT WAS CHANGED AGAINST REIMAGINED'S COPY, so this is an adaptation and
--  not a bulk copy:
--    1. GetMapDisplayName() falls back to the stock map name if the location
--       lookup yields nothing, so the title can never come out blank on a
--       location Treyarch left out of the table.
--    2. Reimagined's "zsr" round-number branch is gone - zsr is their own
--       gametype and zm_qol does not ship it.
--    3. 🛑 STOCK'S TEAM COLOURS ARE RESTORED. Reimagined paints the classic
--       group with Engine.GetFactionColor("zombie"); stock uses
--       CoD.Zombie.SingleTeamColor, and stock has a survival-group branch for
--       Turned that Reimagined dropped. Both are put back, so owning this file
--       changes the TITLE and nothing the player can see anywhere else.
-- ============================================================================
...
-- zm_qol - element 3 of the title. In CLASSIC the map name is the right answer
-- ("Green Run"); in every other zombies gametype the START LOCATION is, which
-- is what turns "Survival - Green Run" into "Survival - Diner".
--
-- Section 5 of zm/gametypestable.csv is the start-location table: column 3 is
-- the location key that ui_zm_mapstartlocation holds, column 16 is its display
-- string. Both the diner row and ZMUI_DINER are stock - dumped out of
-- patch_zm.ff and en_patch_zm.ff to check, rather than added here.
--
...
-- zm\gametypestable.csv (dumped and read 2026-09-02: power, tunnel and the three
-- Die Rise locations are absent; diner/cellblock/street have stock rows and keep
...
function GetMapDisplayName()
	...
	if location ~= nil and location ~= "" then
		local LocationKey = UIExpression.TableLookup(nil, CoD.gametypesTable, 0, 5, 3, location, 16)
		if LocationKey ~= nil and LocationKey ~= "" then
			local LocationName = Engine.Localize(LocationKey)
			-- Engine.Localize hands the key straight back when the string is
			-- missing, which is how a bad key is told from a good one.
			if LocationName ~= nil and LocationName ~= "" and string.match(LocationName, LocationKey) == nil then
				return LocationName
			end
		end
	end
	...
	for Key, ScoreboardTeam in ipairs(ScoreboardTeams) do
		...
		if ScoreboardWidget.teamElements[TeamElementIndex] and ShowScoreboardTeam then
			...
			if FactionTeam then
				...
				-- zm_qol - STOCK's colours, put back. Reimagined paints the
				-- classic group with GetFactionColor("zombie") and has no
				-- survival-group branch at all; stock (patch_zm scoreboard.lua,
				-- the readable part of the decompile) uses SingleTeamColor for
				-- classic and darkens the zombie team in Turned. Restored so
				...
				if CoD.isZombie == true then
					...
					elseif GamemodeGroup == CoD.Zombie.GAMETYPEGROUP_ZSURVIVAL then
						if CoD.Zombie.GAMETYPE_ZCLEANSED == Dvar.ui_gametype:get() and ScoreboardTeam.team == CoD.TEAM_AXIS then
							FactionColorR = CoD.Zombie.SingleTeamColor.r
							FactionColorG = CoD.Zombie.SingleTeamColor.g
							FactionColorB = CoD.Zombie.SingleTeamColor.b
						end
