...
def pixel_pass(port, unlinker, bo2, errors):
    ...
    if not donor or not match:
        errors.append(f"{name}: --pixels needs donorZone and imageMatch in the contract")
        return
    ...
    print(f"[weapon-port] {name}: {len(images)} donor image(s) checked for pixels")
